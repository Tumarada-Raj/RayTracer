...
void tryenvmapper(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2) {
    ...
    //environ map, black wall is removed - rays towards back wall should be mapped as environment, set to green
    ConstantTexture* envtex = new ConstantTexture(RGBColor(0.0f, 1.f, 0.f));
    FlatMaterial envMaterial(envtex);
    EnvironmentMapper* envMapper = new EnvironmentMapper();
    scene.add(new EnvironmentSolid(envMapper, &envMaterial));
