...
public:
    ...
    }					//append bbox to an existing bbox or generate an empty node

    typedef std::vector<Primitive*> Primitives;  //Vector to store primitives
...
public:
    ...
    BVH();
    ~BVH();
    bool isSAH = true; //Flag for SAH enabling/disabling 
    ...
    virtual void buildTree(BVHNode *root);

    float getCenterPoint(int axis, BBox bbox);
